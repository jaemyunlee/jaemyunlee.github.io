...
CURATED_HEADLINES = {
    ...
    ('lesson-01', 22): ('"~하려고 애쓰다/찾아보다"', '일상에서 가장 자연스럽게 쓰는 표현은?'),
    ('lesson-01', 23): ('"먼 길을 끝까지/쭉 가다"', '원어민이 즐겨 쓰는 거리 표현은?'),
    ('lesson-01', 24): ('"티켓을 한꺼번에 다 사다"', '"동시에/한꺼번에"는 영어로?'),
    ('lesson-01', 25): ('"미리/사전에 전화했어야 했는데"', '"미리, 사전에"를 뜻하는 필수 숙어는?'),
    ('lesson-01', 26): ('"미리 전화를 해뒀어야 했는데"', '구어체로 즐겨 쓰는 "미리"는?'),
    ...
    ('lesson-02', 20): ('"예전에 산 위의 폐광산을 탐험하곤 했어요"', '예전에 ~하곤 했다는 영어로?'),
    ('lesson-02', 21): ('"제가 준비한 이야기는 여기까지예요"', '원어민처럼 깔끔하게 끝맺는 표현은?'),
    ('lesson-03', 1): ('"자기주관이 뚜렷하다, 고집 세다"', '영어로 뭐라고 할까요?'),
    ('lesson-03', 2): ('"학교에 차로 데리러 가다"', '일상 필수 영어 표현은?'),
    ('lesson-03', 3): ('"멈추지 않고 끝도 없이 계속하다"', '영어로 뭐라고 할까요?'),
    ('lesson-03', 4): ('"너 상대방 속 뒤집어놓는 법을 아는구나"', '영어 관용구는?'),
    ('lesson-03', 5): ('"차를 길가에 대다, 정차하다"', '운전 필수 영어 표현은?'),
    ('lesson-03', 6): ('"결국 ~하게 되다"', '원어민이 가장 즐겨쓰는 구동사는?'),
    ('lesson-03', 7): ('"절벽 길을 따라서 걷다"', '자연스러운 영어 표현은?'),
    ('lesson-03', 8): ('"아이에게 미아 방지 끈을 채우다"', '영어로 뭐라고 부를까요?'),
    ('lesson-03', 9): ('"완고하고 고집불통인 성격"', '성격을 나타내는 영어 단어는?'),
    ('lesson-03', 10): ('"아직 기저귀를 차고 있는 시절"', '원어민 실생활 표현은?'),
    ('lesson-03', 11): ('"한 번 갈 때 2주씩 머물다"', '한 번에를 뜻하는 영어 표현은?'),
    ('lesson-03', 12): ('"대학 진학으로 집을 떠나다"', '원어민 핵심 구동사는?'),
    ('lesson-03', 13): ('"사위(딸의 남편)"', '영어 가족 호칭으로 뭐라고 부를까요?'),
    ('lesson-04', 1): ('"군중 속에서 유독 눈에 띄다"', '영어로 뭐라고 할까요?'),
    ('lesson-04', 2): ('"분위기에 어울리다, 딱 맞다"', '일상 필수 동사는?'),
    ('lesson-04', 3): ('"생각보다 꽤 많은 수"', '수량을 나타내는 영어 표현은?'),
    ('lesson-04', 4): ('"콘서트 굿즈 판매 구역/코너"', '영어로 뭐라고 부를까요?'),
    ('lesson-04', 5): ('"남들이 안 찾는 애매한 시간대"', '원어민이 쓰는 영어 표현은?'),
    ('lesson-04', 6): ('"하늘석(맨 꼭대기 좌석)"을', '영어로 뭐라고 할까요?'),
    ('lesson-04', 7): ('"지붕이 없는 야외 경기장"', '영어로 뭐라고 부를까요?'),
    ('lesson-04', 8): ('"대본대로 짜인 연출/설정"', '영어로 어떻게 말할까요?'),
    ('lesson-04', 9): ('"그건 온전히 우리에게 달려 있다"', '원어민 필수 관용구는?'),
    ('lesson-04', 10): ('"능청스럽게 분위기를 띄우며 장난치다"', '원어민 핵심 구동사는?'),
    ('lesson-04', 11): ('"사람의 겉모습, 외모"', '외모를 뜻하는 핵심 영어 단어는?'),
    ('lesson-04', 12): ('"알고 보니 ~인 것으로 드러나다"', '일상에서 매일 쓰는 영어 표현은?'),
    ('lesson-04', 13): ('"어떤 인상이나 느낌으로 다가오다"', '세련된 원어민 구동사는?'),
    ('lesson-04', 14): ('"다른 무언가로 변하다, 바뀌다"', '영어로 뭐라고 할까요?'),
    ('lesson-04', 15): ('"뜬금없고 생뚱맞게 느껴지다"', '원어민 일상 표현은?'),
    ('lesson-04', 16): ('"차례를 기다리며 줄을 서다"', '영어로 뭐라고 할까요?'),
    ('lesson-04', 17): ('"맨 첫 번째로 줄을 선"', '일상 필수 영어 표현은?'),
    ('lesson-04', 18): ('"~할 무렵에는, ~할 때쯤에"', '매일 쓰는 접속사구는?'),
    ('lesson-04', 19): ('"콘서트 내내 줄곧"', '필수 전치사 표현은?'),
    ('lesson-04', 20): ('"~인지 아닌지 궁금해하다"', '일상 대화 핵심 패턴은?'),
    ('lesson-04', 21): ('"공연의 마지막 끝 무렵에"', '영어로 뭐라고 할까요?'),
    ('lesson-05', 1): ('"머리가 멍해서 생각이 안 돌아가요"', '영어로 뭐라고 할까요?'),
    ('lesson-05', 2): ('"대학 부전공으로 이수하다"', '영어 단어는?'),
    ('lesson-05', 3): ('"~와는 전혀 관련이 없다"', '필수 숙어 표현은?'),
    ('lesson-05', 4): ('"대학 프로그램에 입학/합격하다"', '원어민 구동사는?'),
    ('lesson-05', 5): ('"대학 공식 성적 증명서"', '영어로 뭐라고 부를까요?'),
    ('lesson-05', 6): ('"요구 조건/자격 요건에 부합하다"', '자연스러운 영어 표현은?'),
    ('lesson-05', 7): ('"시험이 여러 과목을 다루다"', '필수 동사는?'),
    ('lesson-05', 8): ('"선수 과목, 필수 선행 과목"', '교육 행정 핵심 단어는?'),
    ('lesson-05', 9): ('"학점이나 수업을 면제받다"', '고급 영어 동사는?'),
    ('lesson-05', 10): ('"학점 요건을 대체하여 인정받다"', '유용한 일상 동사는?'),
    ('lesson-05', 11): ('"또한, 마찬가지로, 역시"', '문장 끝에 쓰는 일상 표현은?'),
    ('lesson-05', 12): ('"시험 문제로 출제되다, 나오다"', '핵심 구동사는?'),
    ('lesson-05', 13): ('"객관식 (선택형 문제)"을', '영어로 뭐라고 부를까요?'),
    ('lesson-05', 14): ('"내가 쓴 답이나 결정을 뒤늦게 의심하다"', '필수 구어 표현은?'),
    ('lesson-05', 15): ('"나름 괜찮게, 꽤 준수하게 해내다"', '알아두면 유용한 부사는?'),
    ('lesson-05', 16): ('"3점 만점 중에서, 셋 중에서"', '영어로 어떻게 말할까요?'),
    ('lesson-05', 17): ('"어떻게든 무언가를 생각해내다/지어내다"', '원어민 핵심 구동사는?'),
    ('lesson-05', 18): ('"새로운 사실에 눈을 뜨게 하다"', '생생한 영어 관용구는?'),
    ('lesson-05', 19): ('"심도 있고 깊이 있는 대답"', '깊이 있는을 뜻하는 형용사는?'),
    ('lesson-05', 20): ('"우연히 시작하다, 어쩌다 발을 들이다"', '원어민 일상 구동사는?'),
    ('lesson-06', 1): ('"우리는 결국 사귀게 되었죠"', '결국 ~하게 되다는?'),
    ('lesson-06', 2): ('"여기서 계속 어슬렁거리다"', '일상 필수 구동사는?'),
    ('lesson-06', 3): ('"실용주의자, 현실적인 사람"', '뜻을 가진 영어 단어는?'),
    ('lesson-06', 4): ('"다 같이 저녁 외식을 하러 가다"', '영어로 뭐라고 할까요?'),
    ('lesson-06', 5): ('"먹기 싫은 고추를 한쪽으로 쓱 치워두다"', '영어로 어떻게 말할까요?'),
    ('lesson-06', 6): ('"다른 사람들 앞에서 잘난 척/허세 부리다"', '원어민 필수 구동사는?'),
    ('lesson-06', 7): ('"그 사람과 함께 어울리는 시간을 좋아하다"', '따뜻한 영어 표현은?'),
    ('lesson-06', 8): ('"모두 술집으로 데려가 지다"', '수동태 영어 표현은?'),
    ('lesson-06', 9): ('"그 여러 번의 만남 중 어느 한 번"', '회상할 때 쓰는 표현은?'),
    ('lesson-06', 10): ('"연애를 시작한 지 얼마 안 된 연애 초기"', '영어로 뭐라고 할까요?'),
    ('lesson-06', 11): ('"마티네즈에서 라피엣까지 줄곧 내내"', '먼 거리를 강조할 때는?'),
    ('lesson-06', 12): ('"차를 몰고 코너(모퉁이)를 돌다"', '영어로 뭐라고 할까요?'),
    ('lesson-06', 13): ('"계절, 연중 어느 시기인지"', '뜻을 나타내는 표현은?'),
    ('lesson-06', 14): ('"낮잠을 자거나 책을 읽다"', '낮잠을 자다는 영어로?'),
    ('lesson-06', 15): ('"한순간도 지루할 틈이 없다"', '유명한 영어 관용구는?'),
    ('lesson-06', 16): ('"나사가 헐거워져 삐져나오다"', '건축/수리 구어 표현은?'),
    ('lesson-06', 17): ('"참 성가신 일, 골칫거리야"', '원어민들이 매일 쓰는 표현은?'),
    ('lesson-06', 18): ('"아주 먼 옛날에, 훨씬 전에"', '과거를 회상할 때 쓰는 말은?'),
    ('lesson-06', 19): ('"~라는 사실이 참 놀라웠어요"', '영어 문형 패턴은?'),
    ('lesson-06', 20): ('"평온함을 방해하다, 심기를 건드리다"', '자주 쓰이는 동사는?'),
    ('lesson-06', 21): ('"야생동물의 목격 사례, 출몰"', '뜻을 나타내는 명사는?'),
    ('lesson-06', 22): ('"동파 방지 모드로 꼭 설정해야 해"', '~해야 한다는 영어로?'),
    ('lesson-06', 23): ('"물건 정리하고 잠그는 법을 알아내다"', '핵심 구동사는?'),
    ('lesson-07', 1): ('"가던 나를 불쑥 불러세우다"', '원어민 필수 구동사는?'),
    ('lesson-07', 2): ('"정말 뜬금없다고 생각했죠"', '뜬금없는을 뜻하는 핵심 단어는?'),
    ('lesson-07', 3): ('"한국인의 관점/입장에서"', '관점을 뜻하는 영어 명사는?'),
    ('lesson-07', 4): ('"발효된 배추 (김치)"', '발효된을 뜻하는 영어 단어는?'),
    ('lesson-07', 5): ('"거기에 늘 가곤 했어요"', '과거의 습관을 나타낼 때는?'),
    ('lesson-07', 6): ('"격주로, 2주에 한 번꼴로"', '일상 회화 필수 표현은?'),
    ('lesson-07', 7): ('"혹시라도 ~했던 적이 있었다면"', '과거를 회상하는 패턴은?'),
    ('lesson-07', 8): ('"사람들의 시선 집중을 받다"', '빤히 쳐다봄을 당하다는?'),
    ('lesson-07', 9): ('"적어도 두 번은 사줬어요"', '적어도는 영어로?'),
    ('lesson-07', 10): ('"내가 그렇게 부추긴 것 같아요"', '부추기다, 권장하다는?'),
    ('lesson-07', 11): ('"그 맛과 기대에 전혀 미치지 못하다"', '핵심 숙어 표현은?'),
    ('lesson-07', 12): ('"동료 소개로 식당을 알게 되다"', '소개받다를 뜻하는 표현은?'),
    ('lesson-07', 13): ('"딱 잘라 말할 수는 없지만"', '원어민들이 자주 쓰는 패턴은?'),
    ('lesson-07', 14): ('"원래 나야 하는 바로 그 맛"', '핵심 조동사 표현은?'),
    ('lesson-07', 15): ('"예전의 그 맛이 나지 않다"', '맛을 비교할 때 쓰는 말은?'),
    ('lesson-07', 16): ('"엄밀히 따지면, 사실상"', '자주 쓰는 원어민 부사는?'),
    ('lesson-07', 17): ('"그 맛을 바탕(기준)으로 삼다"', '원어민 필수 구동사는?'),
    ('lesson-07', 18): ('"맨 처음 것만큼 맛있지는 않다"', '동등 비교 표현은?'),
    ('lesson-07', 19): ('"결과가 어떻게 완성될지"', '결과가 되다를 뜻하는 구동사는?'),
    ('lesson-07', 20): ('"반면에 그것엔 느끼함이 없었죠"', '대조를 나타내는 접속사는?'),
    ('lesson-07', 21): ('"왠지 나한테 더 기름지게 느껴지다"', '비교급 표현은?'),
    ('lesson-07', 22): ('"김치찌개는 두 달 동안 쉬자"', '휴식기를 갖다는 영어로?')
}
